...
module.exports.World = class World {
    ...
    updatePlayer(dt) {
        this.stage.player.update(dt);
    }

    resolveCollisions() {
        this.stage.enemies.forEach((e) => {
            if (this.player.pos < e.pos) {
                let delta = Math.abs(e.pos - this.player.pos);
                if (delta > 0.1 && delta < PLAYER_LENGTH) {
                    this.logger.debug('Player collided!');
                    if (this.isColorMatch(this.player, e)) {
                        this.logger.debug('Player survided collission!');
                    } else {
                        this.logger.debug('Player LOST ONE LIFE!');
                        this.player.reduceLife();
                        if(this.player.isDead()) {
                            this.logger.debug('Player DEAD!! =============== ');
                        }
                    }
                }
            }
    ...
    isColorMatch(a, b) {
        let colA = colorConvert.rgb.hsl(a.color & 0xff0000, a.color & 0x00ff00, a.color & 0x0000ff)[0]; // only Hue
        let colB = colorConvert.rgb.hsl(b.color & 0xff0000, b.color & 0x00ff00, b.color & 0x0000ff)[0];
        this.logger.debug('COLOR A and B: ' + colA + ' ' + colB);
        return colA === colB;
    }
};