...
module.exports = function dotStarDemoModeRenderer(lines, logger) {
    ...
    lines.forEach((line, idx) => {
        ...
        setPixel(line.pos, calculatePixelColor(line.color));

        /*
        if(idx === 0)
            logger.debug(util.inspect(line));

                    for (let i = line.pos; i < line.pos + line.length; i++) {
            setPixel(i, calculatePixelColor(line.color));
        }
